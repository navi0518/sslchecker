...
function checkSSL() {
  ...
  if (domain === "") {
    displayAlert("Please enter a domain 'exapmle.com'", "danger");
    return;
  }
  ...
  if (!domainPattern.test(domain)) {
    displayAlert("Please enter a valid domain.", "danger");
    return;
  }
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        ...
        displayAlert("Error occurred. Please try again.", "danger");
      }
    }
  };
...
function displayAlert(message, type) {
  ...
  alertMessageDiv.className = "alert alert-" + type; // Set the alert type dynamically
...
function displaySSLInfo(info) {
  ...
  if (info.hasOwnProperty("Error")) {
    ...
    var html =
      "<h2>SSL Certificate Information for " + info["Common Name"] + "</h2>";
